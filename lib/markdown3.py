...
class Markdown(object):
    ...
    def _code_block_sub(self, match, is_fenced_code_block=False):
        ...
        syntax_class_str = self._html_class_str_from_syntax(lexer_name)
        if 0:
            return "\n\n<pre%s %s><code%s>%s\n</code></pre>\n\n" % (
                pre_class_str, syntax_class_str, code_class_str, codeblock)
        return "\n\n<pre%s %s>%s\n</pre>\n\n" % (
            pre_class_str, syntax_class_str, codeblock)

    def _html_class_str_from_syntax(self, name):
        if name is None:
            name = ''
        name = name.strip().lower()
        if 0:
            name = name == 'cpp' and 'c++' or name
            return 'class="lang:%s "'%name
        return 'class="brush:%s"'%name
